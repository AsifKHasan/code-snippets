...
'''
from command line
./salary-summary-for-management.py --month Oct18
'''
import json
import os
import shutil
...
class SalarySummaryForManagement(object):
    ...
    def process_data(self):
        ...
        vals = list(map(lambda v: {"name" : v[2].strip(), "wing" : v[4].strip(), "unit" : v[5].strip(), "grosssalary" : float(re.sub('[ ,-]', '', v[21]) or 0), "benefits" : float(re.sub('[ ,-]', '', v[26]) or 0), "payablethismonth" : float(re.sub('[ ,-]', '', v[46]) or 0)}, vals[5:]))
