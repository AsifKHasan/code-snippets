...
class SqlScriptGenerator(object):
    ...
    _COMPANIES = [
                  {'name': 'sonarbangla', 'id': '36'},
                  {'name': 'continental', 'id': '37'},
                  {'name': 'eastern', 'id': '38'},
                  {'name': 'eastland', 'id': '39'},
                  {'name': 'northern', 'id': '40'},
                  {'name': 'pioneer', 'id': '41'},
                  {'name': 'paramount', 'id': '42'},
                  {'name': 'prime', 'id': '43'},
                  {'name': 'purabi', 'id': '44'},
                  {'name': 'reliance', 'id': '45'},
                  {'name': 'sadharanbima', 'id': '46'},
                  {'name': 'agrani', 'id': '47'},
                  {'name': 'asia', 'id': '48'},
                  {'name': 'meghna', 'id': '49'},
                  {'name': 'asiapacificgen', 'id': '50'},
                  {'name': 'bgcooperatives', 'id': '51'},
                  {'name': 'bggeneral', 'id': '52'},
                  {'name': 'bgnational', 'id': '53'},
                  {'name': 'central', 'id': '54'},
                  {'name': 'citygeneral', 'id': '55'},
                  {'name': 'crystal', 'id': '56'},
                  {'name': 'deshgeneral', 'id': '57'},
                  {'name': 'express', 'id': '58'},
                  {'name': 'federal', 'id': '59'},
                  {'name': 'global', 'id': '60'},
                  {'name': 'greendelta', 'id': '61'},
                  {'name': 'islamicommercial', 'id': '62'},
                  {'name': 'islami', 'id': '63'},
                  {'name': 'janata', 'id': '64'},
                  {'name': 'karnaphuli', 'id': '65'},
                  {'name': 'mercantile', 'id': '66'},
                  {'name': 'nitol', 'id': '67'},
                  {'name': 'peoples', 'id': '68'},
                  {'name': 'phoenix', 'id': '69'},
                  {'name': 'pragati', 'id': '70'},
                  ...
                  {'name': 'republic', 'id': '72'},
                  {'name': 'rupali', 'id': '73'},
                  {'name': 'southasia', 'id': '74'},
                  {'name': 'standard', 'id': '75'},
                  {'name': 'takaful', 'id': '76'},
                  {'name': 'dhaka', 'id': '77'},
                  {'name': 'unionic', 'id': '78'},
                  {'name': 'united', 'id': '79'},
                  {'name': 'senakalyan', 'id': '80'},
                  {'name': 'sikder', 'id': '81'}]
    ...
    def load_script(self, json_file_name):
        ...
        self._GOOGLE_SHEET_NAME = 'Non_Life_Table_State_Data'
        ...
        for row in range[3:]:
            ...
            script = {}
            script_name = row[7] + '__' + row[8] + '.sql'
    ...
    def generate_index_data(self, company, table, range):
        self.sql = range
            ...
            if constraint['unique'] != '-':
                ...
                if constraint['unique'].lower() == 'unique':
                    ...
                    if constraint['composite'].lower() == 'yes':
                        ...
                        self.index_script += 'create unique index ' + unique_name + '_' + table['tableName'] + ' on ' + '"' + company['name'] + '".' + table['tableName'] + ' (' + constraint['columnName'] + ');\n\n'
                else:
                    if constraint['composite'].lower() == 'yes':
                        if constraint['ordinal']:
                        ...
                        if constraint['ordinal'].lower() != '-':
                            unique_name = 'index_' + constraint['columnName']
                            self.index_script += 'create index ' + unique_name + '_' + table['tableName'] + ' on ' + '"' + company['name'] + '".' + table['tableName'] + ' (' + constraint['columnName'] + ');\n\n'
        ...
        for x in self.unique_alphabets_dict:
            if(self.unique_alphabets_dict[x] != ""):
                ...
                self.index_script += 'create unique index ' + unique_name + '_' + uniqueTableName + ' on ' + '"' + company['name'] + '".' + uniqueTableName + ' (' + ', '.join(unique_test) + ');\n\n'
        ...
        for x in self.alphabets_dict:
            if(self.alphabets_dict[x] != ""):
                ...
                self.index_script += 'create index ' + unique_name + '_' + uniqueTableName + ' on ' + '"' + company['name'] + '".' + uniqueTableName + ' (' + ', '.join(test) + ');\n\n'

    def generate_data(self, schema, table, range):
        insert_sql = '-- ' + table['tableName'] + '\n'
        cols = range[0]
        for row in range[1:]:
            insert_sql += 'insert into "' + schema + '".' + table['tableName'] + ' (' + ', '.join(cols) + ') values ('
    ...
    def generate_script(self, deployment_type,  json_file_name):
        ...
        for i, scripts in dict.items():
            for company in self._COMPANIES:
                dir_name = company['id']+'-'+company['name']
                print('Generating script for : '+ dir_name)

                if os.path.exists("./01-table/"+dir_name):
                    shutil.rmtree("./01-table/"+dir_name, ignore_errors=False, onerror=None)
                try:
                    os.makedirs("./01-table/"+dir_name)
                except OSError:
                    pass

                if os.path.exists("./07-insert/"+dir_name):
                    shutil.rmtree("./07-insert/"+dir_name, ignore_errors=False, onerror=None)
                try:
                    os.makedirs("./07-insert/"+dir_name)
                except OSError:
                    pass
                for s, script in enumerate(scripts):
                    sql_table_script = ''
                    ...
                    for t, table in enumerate(tables):
                        ...
                        # Write Table Script
                        sql_table_script += "{:30} {}\n".format('create table',  '"' + company['name'] + '".' + table['tableName'])
                        sql_table_script += '(\n'
                        ...
                        for c, constraint in enumerate(constraints):
                            if constraint['constraintType'].lower() == 'Primary'.lower():
                                ...
                                if len(ref_values) == 3:
                                   constraint_des += "{:40} {:49} {:14} {}".format('', '', 'references', '"' + ref_values[0].strip() + '".' + ref_values[1].strip() + '(' + ref_values[2].strip() + ')')
                                elif len(ref_values) == 2:
                                    constraint_des += "{:40} {:49} {:14} {}".format('', '', 'references', '"' + company['name'] + '".' + ref_values[0].strip() + '(' + ref_values[1].strip() + ')')
                        ...
                        if table['data'] and len(table['data']) > 0:
                            sql_insert_script += self.generate_data(company['name'], table, table['data'])

                        if table['columns']:
                            self.generate_index_data(company, table, table['columns'])

                    # Table Script
                    file_name = 'R__01_' + company['id'] + '_' + script['script']
                    ...
                    if sql_insert_script:
                        file_name = 'R__07_' + company['id'] + '_' + script['script']
                        script_dir = os.path.dirname(os.path.abspath(__file__))
            ...
            if self.index_script:
                file_name = 'R__03_00_02__indexNonLife.sql'
                script_dir = os.path.dirname(os.path.abspath(__file__))
    ...
    def set_up(self):
        self.g = gspread.service_account(filename=self._CREDENTIAL)
