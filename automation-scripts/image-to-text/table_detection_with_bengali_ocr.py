# -*- coding: utf-8 -*-
"""Table-Detection-With-Bengali-OCR.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1nR0s0gu4SLxrplAAXNxSWE_VW4zCqMmX

Instructions:

*   Install All The Dependencies.

*   Restart the Runtime Othewise Code won't work.

*   Upload the input Pdf file in colab, so the pdf file path would be (Ex: "Content/YourFile.pdf").

*   Run the Cells on after the other in Order.

*   The converted Excel files from the detected tables will be automatically downloaded in Zip format.
"""

#@title Install All The Dependencies & Restart The Runtime
...
def read_all_pdf_file_names(pdf_file_path):
    ...
    pdf_file_list = list(filter(lambda file_name: '.pdf' in file_name, all_file_list))
...
def get_all_converted_images_file_names(converted_image_save_directory):
    ...
    image_file_list = list(filter(lambda file_name: '.jpg' in file_name, all_file_list))
...
converted_image_save_directory = '/content/TablePdfConvertedImages'

table_df_save_directory = '/content/TableDataframes'

convert_pdf_to_images(pdf_directory,converted_image_save_directory)

image_file_list = get_all_converted_images_file_names(converted_image_save_directory)
...
for img_file in image_file_list:
  ...
  try:
    ...
    dataframe = extract_table_from_image(converted_image_save_directory+'/'+img_file)

    dataframe.to_excel(table_df_save_directory+'/'+img_file.split('.')[0]+'.xlsx', engine='xlsxwriter')

  except:

    print("Didn't Find Any Table Image")

!zip -r /content/Extracted_Table_Excel_Files.zip /content/TableDataframes
files.download("/content/Extracted_Table_Excel_Files.zip")
