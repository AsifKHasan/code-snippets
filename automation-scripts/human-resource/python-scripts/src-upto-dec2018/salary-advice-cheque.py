#!/usr/bin/env python3
'''
usage: ./salary-advice-cheque.py --month Nov18 --wing Operations
'''
import json
import os
import shutil
...
class SalaryAdviceCash(object):
    ...
    def process_data(self):
        ...
        self._data = list(map(lambda v: {"wing" : v[4].strip(), "unit" : v[5].strip(), "name" : v[2].strip(), "account" : v[9].strip(), "payable" : v[46].strip().replace('-', ''), "paythrough" : v[48].strip(), "paystatus" : v[49].strip()}, vals[5:]))
