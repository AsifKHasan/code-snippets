...
'''
usage:
./salary-advice-bank.py --month Jan19 --refno 18/588
'''
import json
import os
import shutil
...
class SalaryAdviceBank(object):
    ...
    _TEMPLATE = './templates/salary-advice-bank.tex'
    _OUTPUT_PDF = './out/salary-advice-bank__{0}__{1}.pdf'
    _OUTPUT_DATA = './out/salary-advice-bank__{0}__{1}.json'
    ...
    def process_data(self):
        ...
        self._data = list(map(lambda v: {'name' : v[2].strip(), 'unit' : v[5].strip(), 'type' : v[6].strip().replace('-', ''), 'account' : v[9].strip(), 'payable' : v[61].strip().replace('-', ''), 'paythrough' : v[63].strip(), 'paystatus' : v[64].strip()}, vals[5:]))
