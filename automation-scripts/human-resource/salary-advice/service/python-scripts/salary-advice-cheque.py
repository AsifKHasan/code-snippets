...
'''
usage:
./salary-advice-cheque.py --month Jan19 --wing Acquisition
./salary-advice-cheque.py --month Jan19 --wing Delivery
./salary-advice-cheque.py --month Jan19 --wing Operations
'''
import json
import os
import shutil
...
class SalaryAdviceCash(object):
    ...
    _GOOGLE_SHEET_NAME = 'FAU__SalarySheet__2019-2020'
    _TEMPLATE = './templates/salary-advice-cheque.tex'
    _OUTPUT_PDF = './out/salary-advice-cheque__{0}__{1}__{2}.pdf'
    _OUTPUT_DATA = './out/salary-advice-cheque__{0}__{1}__{2}.json'
    ...
    def process_data(self):
        ...
        self._data = list(map(lambda v: {'wing' : v[4].strip(), 'unit' : v[5].strip(), 'name' : v[2].strip(), 'account' : v[9].strip(), 'payable' : v[61].strip().replace('-', ''), 'paythrough' : v[63].strip(), 'paystatus' : v[64].strip()}, vals[5:]))
        self._data = list(filter(lambda v: v['wing'] == self._WING and v['paythrough'] == 'Cheque' and v['paystatus'] == 'In Process', self._data))
