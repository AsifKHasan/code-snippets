...
'''
usage:
./salary-advice-cash.py --month Jan19 --wing Acquisition
./salary-advice-cash.py --month Jan19 --wing Delivery
./salary-advice-cash.py --month Jan19 --wing Operations
'''
import json
import os
import shutil
...
class SalaryAdviceCash(object):
    ...
    def process_data(self):
        ...
        self._data = list(map(lambda v: {'wing' : v[4].strip(), 'unit' : v[5].strip(), 'name' : v[2].strip(), 'payable' : v[61].strip().replace('-', ''), 'paythrough' : v[63].strip(), 'paystatus' : v[64].strip()}, vals[5:]))
