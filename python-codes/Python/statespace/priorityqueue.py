...
# This version of a priority queue was adapted to implement the list protocol,
#  to work with statespace searching.
...
class PriorityQueue:
    ...
    def append(self,v): # was add: now conforms to list protocol
        """
    ...
    def pop(self,which):
        ...
        """
        assert not self.is_empty() and which == 0, 'PriorityQueue:remove empty'
        assert which == 0, 'PriorityQueue:remove can only remove highest priority'
    ...
    def __len__(self): # was size: now conforms to list protocol
        """
...
Command""" 
    ...
    while True:
        ...
        try:
            if   action == 'a': pq.append(prompt.for_string('  Enter value to add'))
            elif action == 'r': print('  remove =',pq.pop())
