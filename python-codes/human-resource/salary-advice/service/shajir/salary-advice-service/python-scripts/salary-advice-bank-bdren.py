#!/usr/bin/env python3
'''
usage: ./salary-advice-bank-bdren.py --month Nov18 --refno 20/568
'''
import json
import os
import shutil
...
class SalaryAdviceBank(object):
    ...
    def process_data(self):
        ...
        self._data = list(map(lambda v: {"name" : v[2].strip(), "type" : v[6].strip().replace('-', ''), "account" : v[9].strip(), "payable" : v[46].strip().replace('-', ''), "paythrough" : v[48].strip(), "paystatus" : v[49].strip()}, vals[5:]))
